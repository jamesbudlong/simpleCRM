                ...
                <div class="table-responsive">
                    ...
                    <tbody>
                        <tr v-for="get_file, index in get_files">
                            ...
                            <td align="center" width="20%"><img style="display:block; width:100%; height:auto;" :src="'/files/uploads/' + get_file.path + '/' + get_file.filename"/> </td>
            ...
            <div v-if="seen2"> <!-- add file form -->
                <div class="container">
                    <div class="container">
                        ...
                        <button class="btn btn-success submitting" v-on:click="submitFiles()" v-show="files.length > 0">Submit</button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        async submitFiles() {
            let app = this;
            let customer_id = app.$route.params.id;
            ...
            for( let i = 0; i < this.files.length; i++ ){
                ...
                await axios.post('/api/v1/files/uploadFile',
                    formData,
                    {
                        headers: {
                    ...
                    await axios.delete('/api/v1/files/' + id)
                        ...
                        .catch(function (resp) {
                            Vue.swal({
                                position: 'center',
                                type: 'error',
                                title: 'Something went wrong! Cannot delete file..',
                                showConfirmButton: false,
                                timer: 2000
                            });
                        });
